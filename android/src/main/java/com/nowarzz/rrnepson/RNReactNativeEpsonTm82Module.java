...
public class RNReactNativeEpsonTm82Module extends ReactContextBaseJavaModule implements PrinterEventListener {
	...
	public void onPrinterClosed(String message) {
		...
		// Toast.makeText(getReactApplicationContext(), "Printer closed: " + message,
	...
	public void printColumn(ReadableArray columnWidths, ReadableArray columnAligns, ReadableArray columnTexts,
		...
		for (int i = 0; i < columnWidths.size(); i++) {
			...
			if (align == Printer.PARAM_DEFAULT)
		...
		if (mBitmap != null) {
			...
			if (width > 0 && height > 0) {
				...
				promise.reject(String.format(
						"Width and height must be more than 0. Width:%s Height:%s Ratio: %s, Bitmap Width: %s, Bitmap Height: %s",
						Integer.toString(width), Integer.toString(height), Double.toString(ratio),
						Integer.toString(mBitmap.getWidth()), Integer.toString(mBitmap.getHeight())));
			}
		} else {
			promise.resolve(null);
		}
