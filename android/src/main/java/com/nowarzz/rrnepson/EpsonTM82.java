...
public class EpsonTM82 implements MyPrinter, ReceiveListener {
	...
	public EpsonTM82(final Context context, final PrinterEventListener eventListener) {
		...
		try {
			this.mPrinter = new Printer(Printer.TM_T88, Printer.MODEL_SOUTHASIA, context);
		...
		this.listener.onInitializeSuccess("TM_T88 Success Connected");
	}
	...
	public MyReturnValue writeText(String text, @Nullable ReadableMap property) {
		...
		try {
			this.mPrinter.addTextStyle(Printer.PARAM_UNSPECIFIED, Printer.PARAM_UNSPECIFIED, paramEM,
		...
		res.success = true;
		res.message = "Text Added in memory";
		Toast.makeText(this.reactContext, res.message, 1).show();
		return res;
	}
	...
	public MyReturnValue writeQRCode(String content, @Nullable ReadableMap property) {
		...
		try {
			this.mPrinter.addSymbol(content, Printer.SYMBOL_QRCODE_MODEL_1, Printer.PARAM_DEFAULT,
					Printer.PARAM_UNSPECIFIED, Printer.PARAM_UNSPECIFIED, Printer.PARAM_UNSPECIFIED);
		} catch (Epos2Exception e) {
			String message;
			int errorStatus = e.getErrorStatus();
			switch (errorStatus) {
	...
	public MyReturnValue writeImage(Bitmap data, int x, int y, int width, int height) {
		...
		try {
			this.mPrinter.addImage(data, x, y, width, height, Printer.PARAM_DEFAULT, Printer.PARAM_DEFAULT,
			...
			case Epos2Exception.ERR_PARAM:
				message = "Invalid parameter";
				message += String.format("X:%s , y=%s, width=%s, height=%s", Integer.toString(x), Integer.toString(y),
	...
	public MyReturnValue writeCut() {
		...
		res.success = true;
		res.message = "mPrinter addCut ok";
		Toast.makeText(this.reactContext, res.message, 1).show();
		return res;
	}
	...
	private boolean connectPrinter(String ipAddress) {
		...
		try {
			...
			case Epos2Exception.ERR_TIMEOUT:
				message = "Connection to printer timeouts";
				break;
			case Epos2Exception.ERR_ILLEGAL:
				message = "The printer connection is open";
				break;
			case Epos2Exception.ERR_MEMORY:
				message = "Out of memory";
				break;
			...
			case Epos2Exception.ERR_PROCESSING:
				message = "Unable to run operation";
				break;
	...
	public MyReturnValue startPrint(String ipAddress) {
		...
		if (this.mPrinter == null) {
			res.success = false;
			res.message = "The printer has not been initiated";
			return res;
		}
		if (!connectPrinter(ipAddress)) {
			res.success = false;
			res.message = "Failed to connect to printer";
			return res;
		}
		...
		if (!isPrintable(status)) {
			...
			res.success = false;
			res.message = "The printer is not ready to print";
			return res;
		}
		try {
